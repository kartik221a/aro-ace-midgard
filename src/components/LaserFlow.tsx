...
interface LaserFlowProps {
  ...
  wispDensity?: number; // Density of the wisps
  dpr?: number; // Optional device pixel ratio override
  mouseSmoothTime?: number; // Smoothing time for mouse movement
  mouseTiltStrength?: number; // Strength of the tilt effect
  horizontalBeamOffset?: number; // Horizontal offset of the beam
  verticalBeamOffset?: number; // Vertical offset of the beam
  flowSpeed?: number; // Speed of the flow animation
  verticalSizing?: number; // Vertical sizing of the beam
  horizontalSizing?: number; // Horizontal sizing of the beam
  fogIntensity?: number; // Intensity of the fog
  fogScale?: number; // Scale of the fog noise
  wispSpeed?: number; // Speed of the wisps
  wispIntensity?: number; // Intensity of the wisps
  flowStrength?: number; // Strength of the flow effect
  decay?: number; // Decay factor for the beam
  falloffStart?: number; // Start of the falloff
  fogFallSpeed?: number; // Speed of the fog fall
...
const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  // Performance measurement for auto-DPR
  const fpsSamplesRef = useRef<number[]>([]);
  const lastFpsCheckRef = useRef<number>(performance.now());
  const emaDtRef = useRef<number>(16.7); // start around 60fps guess
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      antialias: false,
      alpha: false, // Opaque for performance, shader handles black bg
      depth: false,
      stencil: false,
